...
# --- INÍCIO DA MUDANÇA (Constantes de Layout) ---

# Define as áreas da tela
TOP_MARGIN_Y = 100  # Espaço acima dos grids (para IP, títulos, eixos)
BOTTOM_MARGIN_Y = 150  

# Calcula a altura e largura do grid
GRID_HEIGHT = (CELL_SIZE + MARGIN) * 10 + MARGIN
GRID_WIDTH = GRID_HEIGHT  # É um quadrado

# Calcula a altura total da tela
...
class BatalhaNavalPygame:
        ...
        try:
            while not self.callback_queue.empty():
                ...
                if tipo == "novo_participante" or tipo == "lista_participantes":
                    ...
                    self.status_msg = f"Novo(s) oponente(s)! Pressione 'A' para atirar."
                    ...
                    if ip_vitima in self.grids_oponentes:
                        simbolo = self.grid.SYM_AGUA
                        if resultado in ["hit", "destroyed"]:
                            simbolo = self.grid.SYM_ATINGIDO
                            self.grid.score_jogadores_que_atingi.add(ip_vitima)
                        elif resultado == "miss":
                            simbolo = self.grid.SYM_ERRO
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if self.estado_jogo == ESTADO_ATIRANDO:
                    offset_oponente_x = GRID_WIDTH + 100
                    offset_oponente_y = 50
                    x, y = self.get_coord_from_mouse(
                        event.pos, offset_oponente_x, offset_oponente_y
                    )

                    if x is not None:
                        print(f"[JOGO] Atirando em {self.ip_alvo_atual} em ({x},{y})")
                        self.p2p_node.enviar_tiro(self.ip_alvo_atual, x, y)
    ...
    def draw_grid(self, grid_data, offset_x, offset_y, title):
        ...
        for y in range(Grid.GRID_SIZE):
            for x in range(Grid.GRID_SIZE):
                ...
                if celula == self.grid.SYM_AGUA:
                    cor = AZUL
                elif celula == self.grid.SYM_ERRO:
                    cor = BRANCO
                elif celula == self.grid.SYM_ATINGIDO:
                    cor = VERMELHO
                elif celula != self.grid.SYM_AGUA:
                    cor = CINZA
    ...
    def draw_ui(self):
        ...
        # 4. Desenha a Lista de Oponentes
        oponentes_y_start = dashboard_y_start + 40  # Abaixo do status
